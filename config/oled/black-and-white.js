const CONFIG = {
    ...
    CRACKS_PER_100K_PIXELS:  0.8,      // Initial cracks per 100k pixels of screen area (controls pattern density)
    MIN_INITIAL_CRACKS:      2,        // Minimum number of cracks at startup
    MAX_INITIAL_CRACKS:      15,       // Maximum number of cracks at startup
    MAX_CRACKS:              80,       // Maximum total cracks allowed simultaneously (lower = better performance)
    CIRCLE_PERCENT:          35,       // Percentage chance (0-100) that a crack will be curved instead of straight
    
    // VISUAL EFFECTS
    GRAINS:                  50,       // Number of sand grains per region (0 = no sand, higher = more detail but slower)
    SAND_MODE:               'both',   // Sand drawing mode: 'both' (both sides), 'one' (one side), 'none' (disabled)
    SAND_ALPHA:              0.08,     // Maximum opacity of sand grains (0.0-1.0, higher = more visible/vibrant colors)
    ...
    COVERAGE_RESET_PERCENT:  8,        // Canvas coverage % before reset (0-100, lower = more frequent resets)
    ...
    LINE_DRIFT_AMOUNT:       1,        // Maximum angle change in degrees per drift event
    LINE_DRIFT_FREQUENCY:    0.2,      // Probability (0-1) of drift occurring each frame
    
    // SPARKLER EFFECT
    SPARKLER_ENABLED:        true,     // Enable particle sparkles along crack lines
    SPARK_SPAWN_RATE:        2,        // Number of sparks spawned per frame per crack
    SPARK_SPEED_MIN:         0.4,      // Minimum spark velocity
    SPARK_SPEED_MAX:         1.8,      // Maximum spark velocity
    SPARK_LIFETIME:          12,       // Frames until spark disappears
    SPARK_SIZE:              1.2,      // Size of spark particles in pixels
    SPARK_GLOW:              1.5,      // Glow radius around sparks (0 = no glow)
    ...
    FPS_COUNTER_COLOR:       [200, 200, 200], // RGB color of FPS counter text [R, G, B] (0-255 each)
    TARGET_FPS:              60,       // Maximum framerate limit (0 = unlimited, lower = less CPU/GPU usage)
    
    // COLORS
    BG_COLOR:                [0, 0, 0], // Black background (OLED friendly)
    COLORS: [                           // Monochrome white palette
        [255, 255, 255], // Pure White
        [235, 235, 235], // Very Light Gray
        [245, 245, 245]  // Nearly White
    ]
};